...
- The remaining differences (JSON type without bind-parameter support, `DELETE` requiring `WHERE`, `NULL` literals needing an explicit type) are handled and documented in [`dialects`](../reference/api/sinks.sql.dialects.md) and the `sinks/sql/` code.
