...
before diffing -- the thing worth proving here, beyond the plain
insert/touch/rewrite/delete cycle already covered for full catalogs, is
that the merge is real: populating one sweep value never closes out another
value's rows as "missing".
"""
...
def test_swept_catalog_day_by_day_timeline(sync_fn, attr, table, populated_value, empty_value, mutate_field):
    ...
    # Day 1: first run -- every sweep value is fetched (asserted below), only
    # the populated one contributes rows
    ...
    # Day 5: a previously-empty sweep value gets its first row -- inserted,
    # and critically does NOT close out the other value's current rows
    ...
    assert stats["closed"] == 0  # the populated value's rows must be untouched
...
def test_swept_catalog_sweeps_every_declared_value(sync_fn, attr, table, populated_value, empty_value, mutate_field):
    ...
    the fixture happens to populate -- this is the "additional filters/
