...
- **Unstable beneficiary name in `grandesbeneficiarios_busqueda`.** For one `idPersona`, the API returns a different spelling of the name from one hour to the next, with the rest of the record identical. It is not per-request randomness: three consecutive fetches within four minutes returned all 148,170 names identical, yet 79,000 rows changed between the midnight run and the mid-afternoon one, which points to a cache or a periodic re-aggregation at the source. Six variants of the same company name were observed over eleven days (`M&M, S.L.`, `M M S.L.`, `MM SL`, `M&M S.L.`, `M&M SOCIEDAD LIMITADA`, with and without a trailing dot), always with the same amount. The name is probably assembled from the underlying grant records, where each granting body typed it its own way. Hashing that field re-versioned between 30% and 60% of the table every day: 3.7M history rows for 148,000 current records, 25 versions per key in 53 days. The field is excluded from the hash (`exclude_from_hash` in the syncer); it is still stored whole in the payload, it just stops counting as a change. No other entity behaves this way: in `concesiones_busqueda` the high versioning rate is real (amounts accumulating over time).
...
- **Pagination instability against dates still receiving new registrations.** If the underlying result set changes (new grants registered) while a wide window is being paginated, offset-based pagination can repeat rows near a page boundary across two consecutive pages. `bdns-sync` deduplicates at insertion time, so normal syncs leave no duplicates; a large historical load can leave a residual pair. Only affects recent dates, never ranges already closed. How to detect and clean it: [`data-caveats.en.md`](data-caveats.en.md).
