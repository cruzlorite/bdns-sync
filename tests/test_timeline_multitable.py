...
test_timeline_full_catalogs.py, not repeated here. This file only covers
what's specific to the multi-table wiring: the dynamic anios sweep, and the
discover-then-detail/vigencia fan-out.
"""
...
def test_grandesbeneficiarios_busqueda_day_by_day_timeline():
    ...
    # Day 3: upstream corrects one beneficiary's total -- rewrite
    ...
    # Day 5: a new year appears in the _anios catalog -- anios sweep must
    # pick it up dynamically (not a hardcoded year list), and a beneficiary
    # registered under that year is a plain insert
...
def test_planesestrategicos_detail_day_by_day_timeline():
    ...
    # Day 4: a plan disappears from discovery entirely -- full
    # reconciliation closes it out (discover-then-detail still reconciles
    # against the full discovered set every run, same as any other full
    # catalog)
